# Deploy Tanzu for Kubernetes Operations on vSphere with NSX-T
...
This document provides step-by-step instructions for installing and configuring Tanzu for Kubernetes Operations on a vSphere environment backed by a NSX-T based on the reference architecture defined [here](https://docs.google.com/document/d/1z0o006TYhbmCnMSPOC4VRhfYjK-OL6FIff7-JfdtI-c/edit?usp=sharing).

The scope of the document is limited to provide step-by-step instructions for installing and configuring Tanzu for Kubernetes Operations on a vSphere environment backed by a NSX-T based on the reference architecture, and does not cover any deployment procedures for the underlying SDDC components.
...
![](./img/tko-on-vsphere-nsxt/image47.png)
...
| VMware vSphere ESXi     | 7.0 U2a and later     |
| VMware vCenter (VCSA)   | 7.0 U2c and later     |
| VMware vSAN             | 7.0 Update2 and later |
| VMware NSX-T Datacenter | 3.1.3 and Later       |
...
The reference design encompasses the generic network architecture for the TKG reference design.  
For deployment of TKG in the vCF or NSX-T backed environment, we build

1.  Separate Tier-1 Gateway connected to uplink Tier-0 Gateway

2.  Overlay-Backed segments for:
    ...
    - TKG Cluster-VIP/Date Network

    - TKG Management VIP/Data Network

    - TKG Workload VIP/Data Network

Below table provides additional information on these network:

| Network Type                    | DHCP Service | Description & Recommendations                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| NSX ALB Management Network      | Optional     | NSX ALB controllers and SEs will be attached to this network. If DHCP is not available, at least one static pool must be created for the network, and IPAM for the SE Interface IP address is managed by NSX ALB Controller                                                                                                                                                                                                                                                                                |
| TKG Management Network          | Yes          | Control plane and worker nodes of TKG Management Cluster will be attached to this network                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| TKG Cluster VIP/Data Network    | No           | Virtual services for Control plane HA of all TKG clusters (Management, Shared service and Workload)</br>Create a static IP Pool while configuring this network in NSX ALB, the static IP pool must have sufficient IPs depending on the number of TKG clusters planned to be deployed in the environment.</br>IPAM for the VIP and SE Interface IP address is managed by NSX ALB Controller                                                                                                                |
| TKG Management VIP/Data Network | No           | Virtual services for all user-managed packages (such as Contour, Harbor, Contour, Prometheus, Grafana) hosted on the Shared service cluster. Refer User-Managed Packages</br>Create a static IP Pool while configuring this network in NSX ALB, the static IP pool must have sufficient IPs depending on the number of applications that require Load balancing service and an IP for each SE connected to this network.</br>IPAM for the VIP and SE Interface IP address is managed by NSX ALB Controller |
| TKG Workload VIP/Data Network   | No           | Virtual services for all applications hosted on the Workload clusters</br>Create a static IP Pool while configuring this network in NSX ALB, the static IP pool must have sufficient IPs depending on the number of applications that require Load balancing service and an IP for each SE connected to this network.</br>IPAM for the VIP and SE Interface IP address is managed by NSX ALB Controller                                                                                                    |

The network reference design can be mapped into this general framework.

![](./img/tko-on-vsphere-nsxt/image90.png)

# Prepare the Environment for Deployment of the Tanzu Kubernetes Operations

Before you start the deployment of the required components, your environment must meet target prerequisites and be in a specific starting state.

## General Requirements

- A VCF based or NSX-T backed environment.

- If working on a non VCF environment, ensure below NSX-T configurations are in place  
  **Note:** The following steps provides only high level overview of the required NSX-T configuration to be in place, for more details refer [NSX-T Datacenter Installation Guide</span>](https://docs.vmware.com/en/VMware-NSX-T-Data-Center/3.1/installation/GUID-3E0C4CEC-D593-4395-84C4-150CD6285963.html) and [<span class="underline">NSX-T Datacenter Product Documentation](https://docs.vmware.com/en/VMware-NSX-T-Data-Center/index.html)
...
- SDDC environment has the following objects in place:
  ...
  - Depending on the OS flavor of the bootstrap VM, [download](https://customerconnect.vmware.com/en/downloads/details?downloadGroup=TKG-140&productId=988&rPId=49705) and configure the below packages. As part of this documentation, refer to the section “[Deploy and Configure bootstrap machine](#deploy-and-configure-bootstrap-machine)” to configure required packages on Photon Machine

    - Tanzu CLI 1.4.0

    - kubectl cluster CLI 1.21.2

  - A vSphere account with permissions at least described in [Required Permissions for the vSphere Account](https://docs.vmware.com/en/VMware-Tanzu-Kubernetes-Grid/1.4/vmware-tanzu-kubernetes-grid-14/GUID-mgmt-clusters-vsphere.html#vsphere-permissions).
  ...
  - [Download](https://customerconnect.vmware.com/en/downloads/details?downloadGroup=TKG-140&productId=988&rPId=49705) below OVA and import it to vCenter, once imported convert these VM as templates.
...
To configure the firewall, you need to gather the following:

**Note:** Within NSX-T you can make use of DFW and/or Gateway Firewall to configure below mentioned firewall rules. Configuring Firewall on NSX-T is outside the scope of this document, please refer [NSX-T documentation](https://docs.vmware.com/en/VMware-NSX-T-Data-Center/index.html) for more details on configuring firewall.

1.  NSX Advanced Load Balancer Management Network CIDR

2.  TKG Management Cluster Network CIDR

3.  TKG Cluster VIP Network CIDR

4.  TKG Management VIP Network CIDR

5.  TKG Workload Cluster CIDR
...
| TKG Management and Workload Cluster Network      | Harbor Registry                            | TCP:443                        | Allows components to retrieve container images.</br>This registry can be a local or a public image registry (projects.registry.vmware.com)    |
...
| TKG Workload Cluster Network                     | Pinniped VIP</br>(TKG Cluster VIP Network) | TCP:443                        | Allow Pinniped concierge on workload cluster to access Pinniped supervisor on management cluster which is running behind LoadBalancer service |
| TKG Management and TKG Workload Networks         | NSX ALB Controllers                        | TCP:443                        | Allow Avi Kubernetes Operator (AKO) and AKO Operator (AKOO) access to Avi Controller                                                          |
...
| Admin Network                                    | NSX ALB Management Network                 | TCP:443                        | To access and configure NSX Advanced Load Balancer                                                                                            |
| Developer Network/Machine                        | TKG Cluster VIP Network                    | TCP: 6443                      | To access Workload Kubernetes cluster API                                                                                                     |
| Any or custom network                            | TKG workload VIP Network                   | As per application requirement | To access the applications hosted in kubernetes workload clusters, and exposed via load balancing service                                     |
...
- [Configure T1 and Logical Segments in NSX-T Data Center](#configure-t1-gateway-and-logical-segments-in-nsx-t-data-center)
...
- [Deploy TKGm Management Cluster](#deploy-tanzu-kubernetes-grid-tkg-management-cluster)

- [Deploy TKGm Shared Service Cluster](#deploy-tanzu-shared-service-cluster)

- [Deploy TKGm Workload Cluster](#deploy-tanzu-workload-clusters)

- [Deploy User-Managed packages on TKG clusters](#deploy-user-managed-packages-on-tkg-clusters)

Before you start with the deployment, ensure that required Resource pools and folders are created.  

Below are the sample entries of the resource pools and folders that need to be created, change the Portgroup Name, vLAN ID and CIDRs are per your environment.
...
The following is an example of resource pool and folders created:  
...
As part of the pre-requisite, an NSX-T backed vSphere environment backed must be configured with at least one tier-0 Gateway.  
...
1.  Required Overlay Backed Segments

2.  Add a Tier-1 Gateway

3.  Create Overlay backed Segments

### Required Overlay Backed Segments

The below table provides sample entries of the required overlay-backed logical segments, change the Segment Name, and CIDRs are per your environment.

| Network Type               | Sample Segment Name      | Sample Gateway CIDR | DHCP Enabled | DHCP Range configured in NSXT | Static IP Pool reserved for NSX ALB SE/VIP |
| -------------------------- | ------------------------ | ------------------- | ------------ | ----------------------------- | ------------------------------------------ |
| NSX ALB Mgmt Network       | alb-management-segment   | 172.16.10.1/24      | No           | N/A                           | 172.16.10.100 - 172.16.10.200              |
...
| TKG Mgmt VIP Network       | tkg-mgmt-vip-segment     | 172.16.50.1/24      | No           | N/A                           | 172.16.50.100 - 172.16.50.200              |
...
10. Click **Route Advertisement** and ensure below routes are enabled:
...
NSX-T datacenter provides option to add two kinds of segments: overlay-backed segments and VLAN-backed segments.  
...
The below table provides sample entries for the minimum required overlay-backed segments to successfully deploy TKO stack, change the Segment Name and CIDRs are per your environment.
...
| TKG Cluster VIP Network  | tkg-cluster-vip-segment  | 172.16.80.1/24      | No           | N/A                           | 172.16.80.100 - 172.16.80.200              |
| TKG Workload VIP Network | tkg-workload-vip-segment | 172.16.70.1/24      | No           | N/A                           | 172.16.70.100 - 172.16.70.200              |
| TKG Workload Segment     | tkg-workload-segment     | 172.16.60.1/24      | Yes          | 172.16.60.100-172.16.60.200   | N/A                                        |

As shown in the above table, some networks require DHCP. You may make use of NSX-T to provide DHCP services for these networks.  
...
DHCP for the Tier-1 gateway may be configured as such:
...
4.  Click on **Set DHCP Configuration**

5.  In the “Set DHCP Configuration” pop-up window, set the “Type” to “DHCP Server”  
    ![](./img/tko-on-vsphere-nsxt/image25.png)

6.  If you have no DHCP profile created, click on the **menu** icon (3 dots) and select **Create New**  
    ![](./img/tko-on-vsphere-nsxt/image38.png)

7.  In the “Create DHCP Profile” page, provide a **name** for DHCP profile, **select** the **edge cluster** and click on **Save**  
    ![](./img/tko-on-vsphere-nsxt/image36.png)

8.  Click **Save** again in “”Set DHCP Configuration” window  
    ![](./img/tko-on-vsphere-nsxt/image8.png)

9.  Now you would see that the DHCP configuration in the Tier-1 is set to Local, click on save and close editing  
    ![](./img/tko-on-vsphere-nsxt/image41.png)

#### Add or Create a Overlay-Backed Segments

As shown in the previous table you will need to create seven overlay-backed logical segments. All these segments will be part of the same overlay transport zone and must be connected to Tier-1 gateway.  
The following procedure provides required details to create one such network which is required for TKO deployment:

1.  With admin privileges, log in to NSX Manager

2.  Select **Networking** - **Segments**

3.  Click **Add Segment**, enter a name for the segment, for example: “tkg-mgmt-segment”

4.  Under “**Connected Gateway**”, select the Tier-1 Gateway created earlier

5.  Select a transport zone, which will be an **overlay Transport Zone **

6.  Enter the **Gateway IP address** of the subnet in a CIDR format, for example “172.16.40.1/24”  
    ![](./img/tko-on-vsphere-nsxt/image101.png)

7.  Click on “**SET DHCP CONFIG**”  
    **Note:** This is required only for “TKG Management Network”, “TKG Shared Network” and “TKG Workload Network”

    - You may note that the “DHCP type” is set to “Gateway DHCP Server” and DHCP Profile is set to the profile created while creating Tier-1 gateway

    - Under **Settings**, **Enable DHCP Config** and provide the **DHCP range**  
      ![](./img/tko-on-vsphere-nsxt/image6.png)

    - Click on **Options**

    - Under “**Select DHCP Option**” choose “**GENERIC OPTIONS**”

    - Click on “**ADD GENERIC OPTION**” and choose “**NTP servers (42)**”, provide the details of NTP server and click on **ADD**

    - Review the details, and click **Apply** to close the “Set DHCP Config” page  
...
Below is the view from vCenter Networking section
...
**Note:** TKO is based on NSX ALB Enterprise License model
...
- **Set the Cluster IP for the NSX ALB controller**  
  Log in to the primary NSX ALB controller Navigate to **Administrator** **Controller** **Nodes,** and click **Edit**. The Edit Controller Configuration popup appears
...
- Now deploy 2nd and 3rd NSX ALB Node, using steps provided [here](#deploy-nsx-advanced-load-balancer)
...
NSX ALB then performs a warm reboot of the cluster. This process can take 2-3 minutes. The configuration of the primary (leader) Controller is synchronized to the new member nodes when the cluster comes online following the reboot.

Once the cluster is successfully formed we should see the below status:  
![](./img/tko-on-vsphere-nsxt/image43.png)

**Note:** Going forward all NSXT ALB configurations will be configured by connecting to the NSX ALB Controller Cluster IP/FQDN
...
- A TKG cluster can be mapped to only one SE group for Application load balancer services  
  Refer [Configure NSX Advanced Load Balancer in TKG Workload Cluster](#configure-nsx-advanced-load-balancer-in-tkg-workload-cluster) for more details on mapping a specific Service engine group to TKG workload cluster
...
5.  Under the **Network** pane, choose the NSX ALB **Management** **Network** : “**alb-management-segment**” for Service Engines and provide a **Static** **IP** **pool** for SEs and VIP and click on **Complete**
...
For advanced configuration click on the Advanced tab, to specify a specific cluster and datastore for service engine placement, to change the AVI SE folder name and Service engine name prefix and, click on **Save**  
...
Below is the snippet of configuring one of the networks, for example: `tkg-cluster-vip-segment`  
...
- Now attach the IPAM profile to the “tanzu-vcenter-01” cloud
  ...
  - Edit the **tanzu-vcenter-01** cloud
  - Under **IPAM Profile** choose the profile created in previous step and **Save** the configuration
...
For the purpose of this document, we will be using a Photon-based virtual machine as a bootstrap server, refer [here](https://docs.vmware.com/en/VMware-Tanzu-Kubernetes-Grid/1.4/vmware-tanzu-kubernetes-grid-14/GUID-install-cli.html) to configure MAC/Windows machines

- Ensure that the bootstrap VM is connected to TKG Management Network `tkg-mgmt-segment`
...
- Execute the following commands to install TKG CLI, Kubectl CLIs and Carvel tools
...
- Photon OS has docker installed by default, use below commands to start the service and enable it to start at boot.
...
- Create an SSH Key Pair: This is required for Tanzu CLI to connect to vSphere from the bootstrap machine.  
  The public key part of the generated key will be passed during the TKG management cluster deployment.
...
Now all the required packages are installed and required configurations are in place in the bootstrap virtual machines, proceed to next section to deploy the TKG management cluster

## Deploy Tanzu Kubernetes Grid (TKG) Management Cluster

After you have performed the steps described in [deploy and configure bootstrap machine](#deploy-and-configure-bootstrap-machine), you can proceed to deploy management cluster.  
...
For the boot strap machine to pull images from private image repository set below environment variable `TKG_CUSTOM_IMAGE_REPOSITORY`
...
The following procedure provides all required steps to deploy TKG management cluster using the Installer Interface:
...
- **Control Plane Endpoint**: This is an optional field, if left blank NSX ALB will assign an IP from the pool `tkg-cluster-vip-segment` we created earlier.  
  If you need to provide an IP, pick an IP address from `tkg-cluster-vip-segment` static IP pools configured in AVI and ensure that the IP address is unused.
...
- On the Kubernetes Network section, select the **TKG Management Network (“tkg-mgmt-segment”)** where the control plane and worker nodes will be placed during management cluster deployment.  
...
- For the no-proxy section:
  ...
  - For TKG Mgmt and workload clusters, localhost, 127.0.0.1, the values of CLUSTER_CIDR and SERVICE_CIDR, .svc, and .svc.cluster.local values are appended along with the user specified values
...
- Select the **OS image** that will be used for the management cluster deployment.  
  **Note**: This list will appear empty if you don’t have a compatible template present in your environment. Refer steps provided in [Import Base Image template for TKG Cluster deployment](#import-base-image-template-for-tkg-cluster-deployment)t  
...
- The installer will automatically set the context to the TKG Management Cluster in the bootstrap machine.  
...
Key config considerations while creating Shared Service cluster config file
...
| `VSPHERE_CONTROL_PLANE_ENDPOINT`    | This is optional, if left blank NSX ALB will assign an IP from the pool `tkg-cluster-vip-segment` we created earlier.</br>If you need to provide an IP, pick an IP address from “TKG Cluster VIP/Data Network” static IP pools configured in NSX ALB and ensure that the IP address is unused.                                                                                                                                                                                                                                               |
...
Upon preparing the cluster configuration file, execute the below command to initiate the cluster deployment
...
Now the shared service cluster is successfully created, you may proceed with deploying the Harbor package. To deploy the packages refer [Deploy User-Managed Packages on TKG Clusters](#deploy-user-managed-packages-on-tkg-clusters)

In order to deploy the Harbor user package, you need to ensure that “cert-manager” and “contour” user packages are installed. Below will be the deployment order
...
You can see that the workload cluster is successfully deployed and AKO pod is deployed on the cluster. You can now [configure SaaS services</span>](#section-5) for the cluster and/or [deploy user managed packages](#deploy-user-managed-packages-on-tkg-clusters) on this cluster.

## Deploy User-Managed Packages on TKG Clusters
...
#### Attaching a Tanzu Kubernetes Cluster Workload/Shared Service Cluster to TMC

Attaching a cluster involves registering the cluster name with the Tanzu Mission Control cluster agent service, and then installing the cluster agent extensions on the cluster.
...
- [Create a proxy configuration object](#create-a-proxy-configuration-object-in-tmc) in TMC If the cluster is sitting behind a proxy for outbound connectivity
...
2.  In the left navigation pane of the Tanzu Mission Control console, click **Clusters**, click **Attach Cluster**.  
    ![](./img/tko-on-vsphere-nsxt/image20.png)
...
3.  Enter a **name** for the cluster and select the **cluster group** in which you want to register the cluster, and enter a name for the cluster. You can optionally provide a description and one or more labels and click **Next  
    ![](./img/tko-on-vsphere-nsxt/image12.png)
...
![](./img/tko-on-vsphere-nsxt/image100.png)
...
6.  Copy the provided command, switch to the bootstrap machine (or any other machine which has kubectl/tmc installed and has connectivity to the cluster, and then run the command.
    ...
    3.  If you attach using a proxy configuration, make sure you have the latest version of the Tanzu Mission Control CLI (tmc) installed, and then run the tmc command, replacing `<kubeconfig>` with the appropriate `kubeconfig` for the cluster.

    4.  If you attach without a proxy configuration, connect to the cluster with kubectl, and then run the kubectl command.

To obtain admin `kubeconfig` of a cluster, execute below command on the bootstrap Machine `tanzu cluster kubeconfig get <cluster name> --admin --export-file <file name.yaml>`
![](./img/tko-on-vsphere-nsxt/image7.png)
...
2.  On the Administration page, click the **Proxy Configuration** tab.  
    ![](./img/tko-on-vsphere-nsxt/image82.png)
...
9.  In **No proxy list**, you can optionally specify a comma-separated list of outbound destinations that must bypass the proxy server.
    ...
    ![](./img/tko-on-vsphere-nsxt/image28.png)
...
10. Click **Create**. You will find that the proxy configuration is added to TMC and this can be used while adding the cluster which is sitting behind the added proxy. Based on your environmental needs, you may add multiple proxies in TMC  
    ![](./img/tko-on-vsphere-nsxt/image45.png)
...
2.  Login to Tanzu Mission Control, In the left navigation pane of the Tanzu Mission Control console, click **Administration** and click on **Integrations** Tile. You will find the available integrations options, and **enable** the **Tanzu Observability** if not yet enabled  
    ![](./img/tko-on-vsphere-nsxt/image59.png)

3.  Under **Administration** switch to **Accounts,** click on **Create Account Credentials** and select **Tanzu Observability credential and provide the**

    6.  Credential Name

    7.  Tanzu Observability URL

    8.  Tanzu Observability API Token obtained in step 1 and click on **Create**
        ![](./img/tko-on-vsphere-nsxt/image64.png)
...
5.  On the cluster page, click on **Add Integration** and select **Tanzu Observability**  
    ![](./img/tko-on-vsphere-nsxt/image1.png)

6.  Select the **Tanzu Observability Credentials** and click **Confirm**  
    ![](./img/tko-on-vsphere-nsxt/image69.png)  
...
9.  Either click on the Tanzu Observability link provided in TMC or Log in to your Tanzu Observability instance (`<instance_name>.wavefront.com`) to ensure that the metrics are being collected in Tanzu Observability  
    ![](./img/tko-on-vsphere-nsxt/image98.png)
    ![](./img/tko-on-vsphere-nsxt/image35.png)
...
3.  In the **Onboard Clusters** panel, enter a name for the cluster that you want to onboard  
    ![](./img/tko-on-vsphere-nsxt/image68.png)
...
7.  Now you must connect to the target TKG workload cluster and run the commands obtained from previous step  
    ...
    ![](./img/tko-on-vsphere-nsxt/image80.png)successfully
...
8.  Once we have all the required objects created on the cluster, the Tanzu Service Mesh console will prompt you to “Install Tanzu Service Mesh on the cluster”. For the purpose of this document TSM will be installed on all namespaces.  
    **Note: **
    ...
    3.  To install Tanzu Service Mesh in all the namespaces, click Install on all Namespaces. The **system namespaces** on the cluster, such as kube-system, kube-public, and istio-system, **are excluded** from Tanzu Service Mesh by default.

    4.  To exclude a specific namespace from Tanzu Service Mesh, click Exclude Namespaces, select Is Exactly from the left drop-down menu under Exclude Namespaces, and then enter or select the name of the namespace from the right drop-down menu.

    5.  You can also specify the name of a namespace that you plan to create in the cluster at some point in the future
        ![](./img/tko-on-vsphere-nsxt/image14.png)

9.  When the installation is complete, Successfully Onboarded appears next to the cluster name.  
    ![](./img/tko-on-vsphere-nsxt/image56.png)
...
1.  **Login** to Tanzu Mission Control via [VMware Cloud Services](https://console.cloud.vmware.com) page, In the left navigation pane of the Tanzu Mission Control console, click **Clusters**, and ensure that the cluster that needs to be onboarded to TSM is attached to TMC.  
    To attached a cluster to TMC, refer [Attaching a Tanzu Kubernetes Cluster Workload/Shared Service Cluster to TMC](#attaching-a-tanzu-kubernete-cluster-workloadshared-service-cluster-to-tmc)

2.  Click on the **Target Cluster** name, under integration click on **Add Integrations**, and select **Tanzu Service Mesh**

    ![](./img/tko-on-vsphere-nsxt/image105.png)
    **Note:** If you attached a cluster that is running behind a proxy server to Tanzu Mission Control and enabled Tanzu Service Mesh on that cluster, Tanzu Mission Control automatically forwards the proxy configuration to Tanzu Service Mesh. The Tanzu Service Mesh agent on the cluster uses the proxy configuration to connect the cluster to Tanzu Service Mesh through the proxy server. You don't need to provide proxy configuration settings for clusters managed by Tanzu Mission Control in Tanzu Service Mesh.
...
3.  In the “Add Tanzu Service Mesh integration”​​ you can choose to install Tanzu Service mesh on all namespaces or exclude any. For the purpose of this document TSM will be installed on all namespaces.  
    **Note:**
    ...
    3.  You can also specify the name of a namespace that you plan to create in the cluster at some point in the future  
        ![](./img/tko-on-vsphere-nsxt/image94.png)
