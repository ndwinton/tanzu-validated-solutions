#  Deploy Tanzu for Kubernetes Operations on vSphere with VDS
...
This document provides step-by-step instructions for installing and configuring Tanzu for Kubernetes Operations on a vSphere environment backed by a Virtual Distributed Switch (VDS) based on the reference architecture defined [here](https://docs.google.com/document/d/1LP5R4oOYFVT1pqUsTwJjhpajUz8z0cpf5Lw0DedMjcE/edit#heading=h.ewp1sfjib6m).

The scope of the document is limited to provide step-by-step instructions for installing and configuring Tanzu for Kubernetes Operations on a vSphere environment backed by a Virtual Distributed Switch (VDS) based on the reference architecture defined [here](https://docs.google.com/document/d/1LP5R4oOYFVT1pqUsTwJjhpajUz8z0cpf5Lw0DedMjcE/edit#heading=h.ewp1sfjib6m) and does not cover any deployment procedures for the underlying SDDC components.
...
![](./img/tko-on-vsphere-vds/image30.png)
...
For deployment of TKG in the vSphere environment, we build separate networks for the TKG management cluster, TKG workload clusters, NSX ALB management, Cluster-VIP network for Control plane HA, TKG Mgmt VIP/Data network, and TKG workload Data/VIP Network. The network reference design can be mapped into this general framework.

![](./img/tko-on-vsphere-vds/image98.png)
...
As shown in the general topology, your environment must meet the below network requirements:
...
| TKG Management Network          | Yes          | Control plane and worker nodes of TKG Management Cluster and Shared service clusters will be attached to this network                                                                                                                                                                                                                                                                                                                                                                                                       |
...
</br>The below table provides sample entries of the required port groups, change the Portgroup Name, vLAN ID and CIDRs are per your environment.

|Network Type|Sample Port Group Name|Sample vLAN
ID|Sample Gateway CIDR|DHCP Enabled|Static IP Pool reserved for NSX ALB SE/VIP|
|--- |--- |--- |--- |--- |--- |
...
Once you have the network pre-requisites met, the network section in your SDDC must have the below port groups created and must look something like below:

![](./img/tko-on-vsphere-vds/image13.png)
...
| -------------------------------------------------------------------------------------- | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| TKG Management and TKG Workload Networks                                               | DNS Server              |
| NTP Server                                                                             | UDP:53                  |
| UDP:123                                                                                | DNS Service             |
...
| Management and Workload Cluster CIDR                                                   | Harbor Registry         | TCP:443                                                                                                                                       | Allows components to retrieve container images.                                      |
...
| To be more specific please configure                                                   |
| TKG Workload Cluster Network                                                           | TKG Cluster VIP Network | TCP:6443                                                                                                                                      | Allow Workload cluster to register with management cluster                           |
| TKG Workload Cluster Network                                                           | Pinniped VIP            |
| (TKG Cluster VIP Network)                                                              | TCP:443                 | Allow Pinniped concierge on workload cluster to access Pinniped supervisor on management cluster which is running behind LoadBalancer service |
...
- On the Select **networks** page, select the network `nsx_alb_management_pg` and click **Next**
...
- Login to NSX ALB **Administration** **Settings** **Licensing,** on licensing page click on **gear** **icon** next to Licensing\*\*  
...
5.  Under the **Network** pane, choose the NSX ALB **Management** **Network** for Service Engines and provide a **Static** **IP** **pool** for SEs and VIP and click on **Complete**
...
- On the Kubernetes Network section, select the **TKG Management Network (“tkg_mgmt_pg”)** where the control plane and worker nodes will be placed during management cluster deployment. Ensure that the network has **DHCP** service enabled  
...
**_tanzu management-cluster get_**
...
You can see that the workload cluster is successfully deployed and AKO pod is deployed on the cluster. You can now [configure SaaS services</span>](#configure-saas-service) for the cluster and/or [<span class="underline">deploy user managed packages](#deploy-user-managed-packages-on-tkg-clusters) on this cluster.\*\*  
\*\*
...
2.  In the left navigation pane of the Tanzu Mission Control console, click **Clusters**, click **Attach Cluster**.  
    ![](./img/tko-on-vsphere-vds/image12.png)
...
3.  Enter a **name** for the cluster and select the **cluster group** in which you want to register the cluster, and enter a name for the cluster. You can optionally provide a description and one or more labels and click **Next  
    ![](./img/tko-on-vsphere-vds/image9.png)
...
![](./img/tko-on-vsphere-vds/image97.png)
...
![](./img/tko-on-vsphere-vds/image2.png)
...
2.  On the Administration page, click the **Proxy Configuration** tab.  
    ![](./img/tko-on-vsphere-vds/image60.png)
...
9.  In **No proxy list**, you can optionally specify a comma-separated list of outbound destinations that must bypass the proxy server.  
    ![](./img/tko-on-vsphere-vds/image10.png)
...
10. Click **Create**. You will find that the proxy configuration is added to TMC and this can be used while adding the cluster which is sitting behind the added proxy. Based on your environmental needs, you may add multiple proxies in TMC  
    ![](./img/tko-on-vsphere-vds/image34.png)
...
2.  Login to Tanzu Mission Control, In the left navigation pane of the Tanzu Mission Control console, click **Administration** and click on **Integrations** Tile. You will find the available integrations options, and **enable** the **Tanzu Observability** if not yet enabled  
    ![](./img/tko-on-vsphere-vds/image39.png)
...
3.  Under **Administration** switch to **Accounts,** click on **Create Account Credentials** and select **Tanzu Observability credential and provide the**
    ...
    **![](./img/tko-on-vsphere-vds/image57.png)
...
5.  On the cluster page, click on **Add Integration** and select **Tanzu Observability**
    ![](./img/tko-on-vsphere-vds/image3.png)

6.  Select the **Tanzu Observability Credentials** and click **Confirm**  
    ![](./img/tko-on-vsphere-vds/image77.png)  
...
9.  Either click on the Tanzu Observability link provided in TMC or Log in to your Tanzu Observability instance (`<instance_name>.wavefront.com`) to ensure that the metrics are being collected in Tanzu Observability   
    ![](./img/tko-on-vsphere-vds/image96.png)
    ![](./img/tko-on-vsphere-vds/image24.png)
...
3.  In the **Onboard Clusters** panel, enter a name for the cluster that you want to onboard  
    ![](./img/tko-on-vsphere-vds/image67.png)
...
7.  Now you must connect to the target TKG workload cluster and run the commands obtained from previous step  
    ...
    ![](./img/tko-on-vsphere-vds/image70.png)successfully
...
2.  Click on the **Target Cluster** name, under integration click on **Add Integrations**, and select **Tanzu Service Mesh**
    ...
    ![](./img/tko-on-vsphere-vds/image89.png)
...
3.  In the “Add Tanzu Service Mesh integration”​​ you can choose to install Tanzu Service mesh on all namespaces or exclude any. For the purpose of this document TSM will be installed on all namespaces.  
    **Note:**
    ...
    ![](./img/tko-on-vsphere-vds/image85.png)
